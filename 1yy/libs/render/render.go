...
type RenderHandler func(res http.ResponseWriter, req *http.Request) Render
...
func NewRender(options ...Options) RenderHandler {
	opt := prepareOptions(options)
	cs := prepareCharset(opt.Charset)
	t := compile(opt)
	bufpool = bpool.NewBufferPool(64)
	return func(res http.ResponseWriter, req *http.Request) Render {
		...
		return &renderer{res, req, tc, opt, cs}
	}
}
