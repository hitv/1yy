...
func (_ *channelDao) GetAllFilter() (data []*ChannelModel, err error) {
	/*rows, err := db.Query("SELECT `id`,`code`,`name`,`parent`,`is_sub`,`is_rec`,`is_filter`,`status`,`type`,`mi_id` FROM `channel` WHERE `parent`=0 AND status=?", enums.StatusEnabled.String())
	if err != nil {
		return
	}
	defer rows.Close()
